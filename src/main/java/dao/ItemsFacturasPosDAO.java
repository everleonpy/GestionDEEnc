...
public class ItemsFacturasPosDAO {
	...
	public static ArrayList<CamposItemsOperacion> listaItems ( long invoiceId, 
	    ...
	    try {
	        //System.out.println("buscando items de: " + cashId + " - " + controlId + " - " + invoiceId);
        	    ...
        	    ArrayList<PosInvoiceItem> itms = ItemsFacturasPosDAO.listaPlana(invoiceId, controlId, cashId, conn);
        	    if (itms != null) {
	...
	public static ArrayList<PosInvoiceItem> listaPlana ( long invoiceId, long controlId, long cashId, Connection conn) {
		...
		try {
            ...
            */
			...
			buffer.append("select DCODINT, DDESPROSER, QTYHANDLING,");
			buffer.append(" QUANTITY, UNITPRICE, DISCOUNTAMOUNT, DISCOUNTPCT,");
			buffer.append(" UNITDISCAMOUNT DDESCITEM, TAXTYPEID, TAXRATE,");
			buffer.append(" PRODUCTID");
			buffer.append(" from POS_EB_INVOICE_ITEMS");
			buffer.append(" where CASHID = ?");
			buffer.append(" and CASHCONTROLID = ?");
			buffer.append(" and TRANSACTIONID = ?");
			...
			while (rs.next()) {
				...
				o.setDiscountAmt(rs.getDouble("DISCOUNTAMOUNT"));
				o.setDiscountPct(rs.getDouble("DISCOUNTPCT"));
				...
				o.setQtyHandling(rs.getString("QTYHANDLING"));
				o.setQuantity(rs.getDouble("QUANTITY"));
				o.setTaxRate(rs.getDouble("TAXRATE"));
				o.setTaxTypeId(rs.getLong("TAXTYPEID"));
				o.setUnitDiscount(0.0);
				if (rs.getDouble("DISCOUNTAMOUNT") > 0.0) {
				    o.setUnitDiscount(rs.getDouble("DISCOUNTAMOUNT") / rs.getDouble("QUANTITY"));
				}
				o.setUnitPrice(rs.getDouble("UNITPRICE"));
				uomCode = 77;
				uomName = "Unidad";
				if (rs.getString("QTYHANDLING").equalsIgnoreCase("PESABLES")) {
					uomCode = 83;
					uomName = "Kilogramos";
				}
