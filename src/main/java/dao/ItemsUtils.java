...
public class ItemsUtils {
	...
	public CamposValoresItem calcItemValues ( double quantity, 
			double unitPrice, 
			double unitDiscount, 
			double itemDiscPct, 
		...
		//
		CamposValoresItem v = new CamposValoresItem();
		...
		CamposDescuentosItem r = new CamposDescuentosItem();
		...
		amount = UtilPOS.appRound(amount, 8);
		r.setdTotOpeItem(BigDecimal.valueOf(amount));
		if (itemExchgRate == 0) {
	...
	public CamposIvaItem calcTaxValues ( double taxRate, double itemAmount ) {
		// Completar los campos del IVA
		CamposIvaItem t = new CamposIvaItem();
		...
		if (taxRate > 0.0) {
			...
			t.setdDesAfecIVA("Gravado IVA");
			vatProportion = 100.0;
		} else {
			t.setiAfecIVA(EXENTO);
			t.setdDesAfecIVA("Exento");
			vatProportion = 0.0;
		}
		// cuanto por ciento del importe paga IVA
		t.setdPropIVA(BigDecimal.valueOf(vatProportion));
		t.setdTasaIVA(Short.valueOf(String.valueOf((int)taxRate)));
		/*
    	 EA008 dTotOpeItem
    	 E733  dPropIVA
    	 E731  iAfecIVA
