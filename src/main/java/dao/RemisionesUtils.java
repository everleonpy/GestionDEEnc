...
public class RemisionesUtils {
	...
	public CamposReceptorDE datosReceptor ( String shipmentType,
			                                String cellPhoneNo,
			                                String customerCode, 
			                                String customerName,
			                                String address, 
		...
		// remisiones de mercaderias a clientes
		if (shipmentType.equalsIgnoreCase("ENVIO-A-CLIENTE")) {
			// iNatRec
		    ...
		    // iTiOpe
		    gDatRec.setiTiOpe(Short.valueOf(operationType));
		    // iTiContRec
		    if (gDatRec.getiNatRec() == CONTRIBUYENTE) {
			    gDatRec.setiTiContRec(PERSONA_JURIDICA);
			    if (personType.equalsIgnoreCase("FISICA")) {
		    ...
		    // dRucRec
			if (taxNumber != null) {
				gDatRec.setdRucRec(taxNumber);
				gDatRec.setdDVRec(checkDigit);
			}
			// iTipIDRec
			...
			// dNomRec
			gDatRec.setdNomRec(customerName);
			// dNomFanRec
			if (alternName != null) {
			...
			if (customerCode != null) {
				gDatRec.setdCodCliente(customerCode);
			}
		}
		// remisiones de mercaderias a locales
		if (shipmentType.equalsIgnoreCase("ENVIO-LOCALES")) {
			// iNatRec
		    gDatRec.setiNatRec(CONTRIBUYENTE);
		    // iTiOpe
		    gDatRec.setiTiOpe(Short.valueOf(operationType));
