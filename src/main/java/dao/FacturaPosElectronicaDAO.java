...
public class FacturaPosElectronicaDAO {
	...
	public static ArrayList<DocumElectronico> getPosInvoices ( java.util.Date trxDate, 
			                                                   int refNumber, 
			                                                   int maxTrxQty, 
			                                                   long orgId ) {
		...
		int trxCounter = 0;
		//
		PosEbInvoice readTx = null;
		PosEbInvoice previousTx = null;
		...
		try {
			...
			org = EmisorDocumentosDAO.getOrgData(orgId);
			...
			buffer.append(" h.EB_CONTROL_CODE, h.IDENTIFIER, h.CASHCONTROLID,");
			buffer.append(" h.CASHID, h.CANCELED_ON,");
			...
			// campos desnormalizados correspondientes a las formas de pago
			buffer.append(" PAYMENTTERM1, PAYMENTAMT1, CARDMARK1, CHECKNUMBER1, CHECKBANK1,"); 
			buffer.append(" PAYMENTTERM2, PAYMENTAMT2, CARDMARK2, CHECKNUMBER2, CHECKBANK2,"); 
			buffer.append(" PAYMENTTERM3, PAYMENTAMT3, CARDMARK3, CHECKNUMBER3, CHECKBANK3,"); 
			buffer.append(" PAYMENTTERM4, PAYMENTAMT4, CARDMARK4, CHECKNUMBER4, CHECKBANK4"); 
			// condiciones de la consulta
			buffer.append(" from POS_EB_INVOICES h,");
			buffer.append(" POS_EB_INVOICE_ITEMS x");
			buffer.append(" where x.CASHID = h.CASHID");
			buffer.append(" and x.CASHCONTROLID = h.CASHCONTROLID");
			...
			buffer.append(" order by h.CASHID, h.CASHCONTROLID, h.IDENTIFIER");
			//
			...
			while (rs.next()) {
				...
				rowsCounter++;
				//
				//System.out.println("Leyendo: " + rs.getLong("IDENTIFIER") + " - " + rs.getLong("CASHCONTROLID") + " - " + rs.getLong("CASHID"));
				readTx = new PosEbInvoice();
				...
				readTx.setCashcontrolid(rs.getLong("CASHCONTROLID"));
				readTx.setCashid(rs.getLong("CASHID"));
				...
				readTx.setPaymentTerm1(rs.getShort("PAYMENTTERM1"));
				readTx.setPaymentAmt1(rs.getDouble("PAYMENTAMT1"));
				readTx.setCardMark1(rs.getShort("CARDMARK1"));
				readTx.setCheckNumber1(rs.getString("CHECKNUMBER1"));
				readTx.setCheckBank1(rs.getString("CHECKBANK1"));

				readTx.setPaymentTerm2(rs.getShort("PAYMENTTERM2"));
				readTx.setPaymentAmt2(rs.getDouble("PAYMENTAMT2"));
				readTx.setCardMark2(rs.getShort("CARDMARK2"));
				readTx.setCheckNumber2(rs.getString("CHECKNUMBER2"));
				readTx.setCheckBank2(rs.getString("CHECKBANK2"));

				readTx.setPaymentTerm3(rs.getShort("PAYMENTTERM3"));
				readTx.setPaymentAmt3(rs.getDouble("PAYMENTAMT3"));
				readTx.setCardMark3(rs.getShort("CARDMARK3"));
				readTx.setCheckNumber3(rs.getString("CHECKNUMBER3"));
				readTx.setCheckBank3(rs.getString("CHECKBANK3"));

				readTx.setPaymentTerm4(rs.getShort("PAYMENTTERM4"));
				readTx.setPaymentAmt4(rs.getDouble("PAYMENTAMT4"));
				readTx.setCardMark4(rs.getShort("CARDMARK4"));
				readTx.setCheckNumber4(rs.getString("CHECKNUMBER4"));
				readTx.setCheckBank4(rs.getString("CHECKBANK4"));

				//
				readItem = new PosEbInvoiceItem();
				readItem.setCashcontrolid(readTx.getCashcontrolid());
				readItem.setCashid(readTx.getCashid());
				...
				if (readTx.getIdentifier() != previousTx.getIdentifier() |
				    readTx.getCashcontrolid() != previousTx.getCashcontrolid() |
				    readTx.getCashid() != previousTx.getCashid()) {
				    //System.out.println("Construyendo lista de pagos: " + previousTx.getIdentifier() );
					// construir la lista de formas de pago
					ArrayList<PaymentTermItem> pmts = buildTermsList ( previousTx );
					...
					if (trxCounter <= maxTrxQty) {
				        //System.out.println("Cargando: " + previousTx.getIdentifier() + " RUC: " + previousTx.getTaxnumber() );
				        rDE = loadDocument( previousTx, org, gCamItem, pmts, conn);
				        System.out.println("Cargado: " + previousTx.getIdentifier() + " CDC: " + previousTx.getControlcode() );
				        deList.add(rDE);
					}
				    if (maxTrxQty > 0) {
					    if (trxCounter >= maxTrxQty) {
	...
	private static ArrayList<PaymentTermItem> buildTermsList ( PosEbInvoice x ) {
		short ZERO_VALUE = 0;
		short EFECTIVO = 1;
		// construir la lista de formas de pago
		ArrayList<PaymentTermItem> pmts = new ArrayList<PaymentTermItem>();
		//System.out.println(x.getPaymentTerm1() + " - " + x.getPaymentTerm2() + " - " +
		//		x.getPaymentTerm3() + " - " + x.getPaymentTerm4());
		if (x.getCanceledOn() == null) {
			if (x.getPaymentTerm1() != ZERO_VALUE) {
				PaymentTermItem t = new PaymentTermItem();
				t.setTermId(x.getPaymentTerm1());
				t.setAmount(x.getPaymentAmt1());
				t.setCardMark(x.getCardMark1());
				t.setCheckNumber(x.getCheckNumber1());
				t.setCheckBank(x.getCheckBank1());
				if (x.getPaymentTerm1() == 3 | x.getPaymentTerm1() == 4) {
					if (x.getCardMark1() == 0) {
						t.setCardMark(Short.valueOf("99"));
					}
				}
				if (x.getPaymentTerm1() == 2) {
					if (x.getCheckNumber1() == null) {
			...
			if (x.getPaymentTerm2() != ZERO_VALUE) {
				PaymentTermItem t = new PaymentTermItem();
				t.setTermId(x.getPaymentTerm2());
				t.setAmount(x.getPaymentAmt2());
				t.setCardMark(x.getCardMark2());
				t.setCheckNumber(x.getCheckNumber2());
				t.setCheckBank(x.getCheckBank2());
				if (x.getPaymentTerm2() == 3 | x.getPaymentTerm2() == 4) {
					if (x.getCardMark2() == 0) {
						t.setCardMark(Short.valueOf("99"));
					}
				}
				if (x.getPaymentTerm2() == 2) {
					if (x.getCheckNumber2() == null) {
						t.setCheckNumber("00000000");
						t.setCheckBank("No Definido");
					}
				}
			...
			if (x.getPaymentTerm3() != ZERO_VALUE) {
				PaymentTermItem t = new PaymentTermItem();
				t.setTermId(x.getPaymentTerm3());
				t.setAmount(x.getPaymentAmt3());
				t.setCardMark(x.getCardMark3());
				t.setCheckNumber(x.getCheckNumber3());
				t.setCheckBank(x.getCheckBank3());
				if (x.getPaymentTerm3() == 3 | x.getPaymentTerm3() == 4) {
					if (x.getCardMark3() == 0) {
						t.setCardMark(Short.valueOf("99"));
					}
				}
				if (x.getPaymentTerm3() == 2) {
					if (x.getCheckNumber3() == null) {
						t.setCheckNumber("00000000");
						t.setCheckBank("No Definido");
					}
				}
			...
			if (x.getPaymentTerm4() != ZERO_VALUE) {
				PaymentTermItem t = new PaymentTermItem();
				t.setTermId(x.getPaymentTerm4());
				t.setAmount(x.getPaymentAmt4());
				t.setCardMark(x.getCardMark4());
				t.setCheckNumber(x.getCheckNumber4());
				t.setCheckBank(x.getCheckBank4());
				if (x.getPaymentTerm4() == 3 | x.getPaymentTerm4() == 4) {
					if (x.getCardMark4() == 0) {
						t.setCardMark(Short.valueOf("99"));
					}
				}
				if (x.getPaymentTerm4() == 2) {
					if (x.getCheckNumber4() == null) {
						t.setCheckNumber("00000000");
						t.setCheckBank("No Definido");
					}
				}
			...
			double amount = PosTransItemsDAO.getInvoiceAmount(x.getIdentifier(), x.getCashcontrolid(), x.getCashid());
			t.setAmount(amount);			
			pmts.add(t);
		}
	...
	private static DocumElectronico loadDocument ( PosEbInvoice p, 
			                                       OrganizationData org, 
	    ...
	    */
	    ...
	    rDE.setDE(DE);
	    rDE.setCashId(p.getCashid());
	    rDE.setControlId(p.getCashcontrolid());
	    rDE.setTransactionId(p.getIdentifier());
		//System.out.println("21a");
	    return rDE;
	}
	
	private static CamposTimbrado setgTimbValues ( PosEbInvoice x) {
		java.text.SimpleDateFormat df = new java.text.SimpleDateFormat("yyyy-MM-dd");
		//
	...
	private static CamposEmisorDE setgEmisValues ( PosEbInvoice x, OrganizationData org) {
	...
	private static ReceiverData setReceiverData ( PosEbInvoice x ) {
	...
	private static CamposOperacionComercial setgOpeComerValues ( PosEbInvoice x ) {
	    // datos de la operacion comercial
	...
	private static CamposCondicionOperacion setCondValues ( PosEbInvoice x, 
			                                                short iTiOpe, 
			                                                long invoiceId, 
			                                                long controlId, 
			                                                long cashId, 
	    ...
	    /**
	     ...
	     +--------------------------------------------------------------+
	     */
		//System.out.println("asignando condicion");
	    ...
	    if (gCamCond.getiCondOpe() == CONTADO) {
		    ...
		    ArrayList<CamposOperacionContado> gPaConEIni = FormasPagoFacturasPosDAO.listaFormasPago ( pmts );
			...
			//System.out.println("procesando operaciones a credito");
