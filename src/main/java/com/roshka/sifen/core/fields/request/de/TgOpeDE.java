...
public class TgOpeDE extends SifenObjectBase {
    ...
    // este es el constructor original de la clase
    public TgOpeDE() {
        this.dCodSeg = SifenUtil.generateRandomNumber();
    }

    // este es un constructor creado para esta implementacion, en base a que en la mayoria de los casos
    // el CDC es creado en un momento previo al envio
    public TgOpeDE(String controlCode) {
	    if (controlCode == null) {
        this.dCodSeg = SifenUtil.generateRandomNumber();
	    } else {
	    	    this.dCodSeg = controlCode.substring(34, 43);
	    }
    }
