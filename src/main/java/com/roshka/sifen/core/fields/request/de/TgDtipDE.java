...
public class TgDtipDE extends SifenObjectBase {
    ...
    public void setupSOAPElements(SOAPElement DE, TTiDE iTiDE, TdDatGralOpe gDatGralOpe) throws SOAPException {
        ...
        if (iTiDE.getVal() == 7 || (iTiDE.getVal() == 1 && this.gTransp != null)) {
        	    ...
        	    if (gDtipDE == null) {
        	    	    System.out.println("gDtipDE es nulo");
        	    }
        	    if (iTiDE == null) {
    	    	        System.out.println("iTiDE es nulo");
    	        }
        	    if (this.gCamNRE == null) {
    	    	        System.out.println("this.gCamNRE es nulo");
    	        } else {
    	        	    if (this.gCamNRE.getiMotEmiNR() == null) {
    	        	    	    System.out.println("this.gCamNRE.getiMotEmiNR() es nulo");
    	        	    }
    	        }
            this.gTransp.setupSOAPElements(gDtipDE, iTiDE, this.gCamNRE != null ? this.gCamNRE.getiMotEmiNR() : null);
        }        
