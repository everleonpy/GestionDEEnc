...
public class TgTotSub extends SifenObjectBase {
    ...
    public void setupSOAPElements(SOAPElement DE, TTiDE iTiDE, TgDtipDE gDtipDE, TgOpeCom gOpeCom) throws SOAPException {
        ...
        for (TgCamItem gCamItem : gDtipDE.getgCamItemList()) {
            ...
            // agregado en junio 23, para redondeo
            if (dTotOpeItem.compareTo(BigDecimal.ZERO) != 0) {
            	    dTotOpeItem = dTotOpeItem.setScale(8, RoundingMode.HALF_UP);
            }
            ...
            // Nota Tecnica No. 1
            // Suma de la multiplicación de cada descuento particular por ítem por la
            // cantidad de ese ítem (sumatoria de EA002 * E711)
            // EA002 -> gValorRestaItem.dDescItem
            // E711  -> gCamItem.dCantProSer
            //System.out.println("acumulando dTotDesc: " + this.dTotDesc + " unitario: " +
            //		gCamItem.getgValorItem().getgValorRestaItem().getdDescItem() +
            //		" cantidad: " + gCamItem.getdCantProSer());
            this.dTotDesc = this.dTotDesc.add(SifenUtil.coalesce(gCamItem.getgValorItem().getgValorRestaItem().getdDescItem(), BigDecimal.ZERO).
            		                              multiply(gCamItem.getdCantProSer())); 
            //System.out.println("resultado dTotDesc: " + this.dTotDesc);
            // nota tecnica No. 1 Oct. 14 2019
            // Si se cuenta con un descuento global, debe ser aplicado (no es prorrateo) a cada uno de los ítems, 
            // independientemente de que un ítem cuente con un descuento particular. [F010 * E721 / 100]
            // F010 -> gTotSub.dPorcDescTotal
            // E721 -> gValorItem.dPUniProSer
            //
            //  Sumatoria de la multiplicación de cada descuento global por ítem por la
            // cantidad de ese ítem (sumatoria de EA004 * E711)
            // EA004 -> gValorRestaItem.dDescGloItem
            // E711  -> gCamItem.dCantProSer
        ...
        // Nota Tecnica No. 1
        //  Sumatoria de todos los descuentos (Global por Ítem y particular por ítem)
        // de cada ítem ( F009 + F033 )
        // F009 -> gTotSub.dTotDesc
        // F033 -> gTotSub.dTotDescGlotem
        this.dDescTotal = this.dTotDesc.add(this.dTotDescGlotem);
        // la siguiente linea es la original
        ...
        // este cambio obecedece a que la especificacion dice que dPorcDescTotal es el porcentaje de descuento global
        // sobre total de la operacion, pero la validacion lo liga a la columna EA004 que es descuento global por item dDescGloItem
        this.dPorcDescTotal = BigDecimal.ZERO;
        if (this.dTotDescGlotem.doubleValue() > 0.0) {
            this.dPorcDescTotal = this.dDescTotal.multiply(BigDecimal.valueOf(100)).divide(this.dTotOpe.add(this.dDescTotal), 2, RoundingMode.HALF_UP);        	
        }
        // Nota Tecnica No. 1
        //  Sumatoria de todos los Anticipos (Global por Ítem y particular por ítem)
        // F034 + F035
        // F034 -> gTotSub.dTotAntItem
        // F035 -> gTotSub.dTotAnt
        this.dAnticipo = this.dTotAntItem.add(this.dTotAnt);
        
        //System.out.println("this.dRedon(antes): " + this.dRedon);
        this.dRedon = this.dTotOpe.subtract(BigDecimal.valueOf(Math.round(this.dTotOpe.doubleValue() * 50) / 50));
        //System.out.println("this.dRedon(despues): " + this.dRedon);
        
        if (this.dRedon.compareTo(BigDecimal.ZERO) == -1) {
        	    this.dRedon = BigDecimal.ZERO;
            //System.out.println("this.dRedon(quedando en): " + this.dRedon);
        }
        
        // esta linea la agregamos para redondear el resultadoa 4 decimales
        this.dRedon = this.dRedon.setScale(4, RoundingMode.HALF_UP);
        // Nota Tecnica Mo. 001
        //  Corresponde al cálculo aritmético: F008 - F013 + F025
        // F008 -> gTotSub.dTotOpe
        // F013 -> gTotSub.dRedon
        // F025 -> gTotSub.dComi
        ...
        // jLcc - Abril 6, 2023
        // esta linea la agregamos para redondear el importe a lo que establece la especificacion
        if (this.dTBasGraIVA.compareTo(BigDecimal.ZERO) != 0) {
            this.dTBasGraIVA = this.dTBasGraIVA.setScale(8, RoundingMode.HALF_UP);
        }
        ...
        // jLcc - Enero 4, 2023
        // esta linea la agregamos para redondear el importe a lo que establece la especificacion
        if (this.dTotOpe.compareTo(BigDecimal.ZERO) != 0) {
            this.dTotOpe = this.dTotOpe.setScale(8, RoundingMode.HALF_UP);
        }
        if (this.dTotDesc.compareTo(BigDecimal.ZERO) != 0) {
            this.dTotDesc = this.dTotDesc.setScale(8, RoundingMode.HALF_UP);
        }
        if (this.dTotDescGlotem.compareTo(BigDecimal.ZERO) != 0) {
            this.dTotDescGlotem = this.dTotDescGlotem.setScale(8, RoundingMode.HALF_UP);
        }
        if (this.dTotAntItem.compareTo(BigDecimal.ZERO) != 0) {
            this.dTotAntItem = this.dTotAntItem.setScale(8, RoundingMode.HALF_UP);
        }
        if (this.dTotAnt.compareTo(BigDecimal.ZERO) != 0) {
            this.dTotAnt = this.dTotAnt.setScale(8, RoundingMode.HALF_UP);
        }
        if (this.dPorcDescTotal.compareTo(BigDecimal.ZERO) != 0) {
            this.dPorcDescTotal = this.dPorcDescTotal.setScale(8, RoundingMode.HALF_UP);
        }
        if (this.dDescTotal.compareTo(BigDecimal.ZERO) != 0) {
            this.dDescTotal = this.dDescTotal.setScale(8, RoundingMode.HALF_UP);
        }
        if (this.dAnticipo.compareTo(BigDecimal.ZERO) != 0) {
            this.dAnticipo = this.dAnticipo.setScale(8, RoundingMode.HALF_UP);
        }
        if (this.dRedon.compareTo(BigDecimal.ZERO) != 0) {
            this.dRedon = this.dRedon.setScale(4, RoundingMode.HALF_UP);
        }
