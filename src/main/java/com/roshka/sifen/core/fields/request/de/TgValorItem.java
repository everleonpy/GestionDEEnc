...
public class TgValorItem extends SifenObjectBase {
    ...
    public void setupSOAPElements(SOAPElement gCamItem, TTiDE iTiDE, TdCondTiCam dCondTiCam, TTImp iTImp, BigDecimal dCantProSer) throws SOAPException {
        ...
        if (this.dPUniProSer.compareTo(BigDecimal.ZERO) != 0) {
            this.dPUniProSer = this.dPUniProSer.setScale(8, RoundingMode.HALF_UP);
        }
        ...
        // jLcc - Enero 14, 2023
        // esta linea la agregamos para redondear el importe a lo que establece la especificacion
        if (this.dTotBruOpeItem.compareTo(BigDecimal.ZERO) != 0) {
            this.dTotBruOpeItem = this.dTotBruOpeItem.setScale(8, RoundingMode.HALF_UP);
        }
