...
public class PosDeliverXml extends JFrame {
	...
	public PosDeliverXml () {
	    ...
	    this.ctrl.initForm();
	    this.ctrl.setEvent("DELIVER-XML");
		// build GUI
		initialize();
		try {		
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			filtersHeight = getRelativeHeight (windowHeight, 22.0);
			filtersX = 0;
			...
			jlAction1.addMouseListener(new MouseAdapter() {  
			    public void mouseClicked(MouseEvent e) {  
			        ...
			        if (rowIndex >= 0) {
			            ...
			            eMailAddress = (String) jaTransactions.getValueAt(rowIndex, 4);
			            txNumber = (String) jaTransactions.getValueAt(rowIndex, 0);
			            txDate = (java.util.Date) jaTransactions.getValueAt(rowIndex, 5);
			            ctrl.setSelectedTransactionId(invoiceId);
			    	        ApplicationMessage m = ctrl.deliverXmlFile ( invoiceId, 
			    	        		                                         controlId, 
			    	        		                                         cashId, 
			    	        		                                         eMailAddress,
			    	        		                                         txNumber, 
	...
	private JPanel getJpFilters() {
		if (jpFilters == null) {
			...
			// primera linea
			int lineNum = 0;
			jlCash = getFiltersLabel("Caja", "RIGHT");
	...
	public static void main(String[] args) {
		...
		//
		PosDeliverXml w = new PosDeliverXml();
		w.setVisible(true);
		w.repaint();
	}
