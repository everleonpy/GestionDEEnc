...
import business.SendRcvInvoicesControllerOld;
...
public class SendRcvInvoicesOld extends JFrame {
	...
	private SendRcvInvoicesControllerOld ctrl;
	...
	public SendRcvInvoicesOld () {
	    ...
	    this.ctrl = new SendRcvInvoicesControllerOld();
		...
		if (jpToolbar == null) {
			...
			jlAction1.addMouseListener(new MouseAdapter() {  
			    public void mouseClicked(MouseEvent e) {  
			        ...
			        if (rowIndex >= 0) {
			            ...
			            invoiceId = (Long) jaTransactions.getValueAt(rowIndex, 9);
			            ctrl.setSelectedTransactionId(invoiceId);
			    	        ApplicationMessage m = ctrl.sendInvoice ( invoiceId, 
			...
			jlAction2.addMouseListener(new MouseAdapter() {  
			    public void mouseClicked(MouseEvent e) {  
		            ...
		            while (itr.hasNext()) {
		        	        RcvInvoice x = (RcvInvoice) itr.next();
		    	            ApplicationMessage m = ctrl.sendInvoice ( x.getInvoiceId(), 
		    	    		                                                  UserAttributes.userOrg.getIDENTIFIER(), 
		    	    		                                                  UserAttributes.userUnit.getIDENTIFIER(), 
		    	    		                                                  UserAttributes.userName );
		    	            counter++;
		            }
	        	        MessageWindow x = new MessageWindow(null, "Envio de Facturas", counter + " transacciones procesadas");
			        x.setVisible(true);			    	        	
			    }  
			    public void mouseEntered (MouseEvent evt) {
	...
	private JPanel getJpFilters() {
		if (jpFilters == null) {
			...
			lineNum++;
			lbY = lbY + lbH;
			jlInvCondition = getFiltersLabel("Condicion");
			jpFilters.add(jlInvCondition, null);
			// tercera linea
			lineNum++;
			lbY = lbY + lbH;
			...
			tfX = startX + lbW + hGap;
			tfY = tfY + tfH;
			jpFilters.add(getjoInvCondition(), null);			
			// tercera linea
			tfX = startX + lbW + hGap;
			tfY = tfY + tfH;
			tfW = tfWidth;
	...
	private JComboBox<String> getjoInvCondition() {
		if (joInvCondition == null) {
			joInvCondition = getFiltersCombobox();
			joInvCondition.setFocusTraversalKeysEnabled(false);
			joInvCondition.addItem("Contado");
			joInvCondition.addItem("Credito");
			joInvCondition.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					ApplicationMessage m = ctrl.enterTxCondition((String)joInvCondition.getSelectedItem());
					jtFromDate.requestFocusInWindow();
				}
			});
		}
		return joInvCondition;
	}
        ...
        if (e.isAltDown() == true) {
			...
			if (e.getKeyCode() == KeyEvent.VK_F9) {
				...
				if (focusOwner instanceof JTextField) {
					...
					if (focusOwner.getName().equalsIgnoreCase("SITE")) {
						showListOfValues ( "SUCURSALES", "Sucursales de la Organizacion", 0, null);
						if (lovSelection != null) {
							ctrl.enterSiteId(lovSelection.getItemId());
							jtSite.setText(lovSelection.getItemName());
	...
	*/
	...
	public static void main(String[] args) {
		...
		//
		SendRcvInvoicesOld w = new SendRcvInvoicesOld();
		w.setVisible(true);
		w.repaint();
	}
