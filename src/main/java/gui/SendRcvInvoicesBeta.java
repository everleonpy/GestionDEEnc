...
import business.RcvInvoicesTM;
import business.SendRcvInvoicesControllerBeta;
...
public class SendRcvInvoicesBeta extends JFrame {
	...
	private SendRcvInvoicesControllerBeta ctrl;
	 ...
	 */
	public SendRcvInvoicesBeta () {
		super();
		// set application theme
		appLook = new AppLookAndFeel("BLUE-SKY");
		//appLook = new AppLookAndFeel("BLUE-SKY");
		// instantiate MVC controller
	    this.ctrl = new SendRcvInvoicesControllerBeta();
		// build GUI
		initialize();
		try {		
		...
		if (jpToolbar == null) {
			...
			jlAction1.addMouseListener(new MouseAdapter() {  
				public void mouseClicked(MouseEvent e) {  
					...
					/*
					// verificar si existen transacciones en la fecha ingresada
					if (ctrl.getInvoicesQty() == 0) {
						ctrl.findInvoicesCount();
					}
					if (ctrl.getPreparedQty() < ctrl.getInvoicesQty()) {
						//sendData = false;
						MessageWindow wm = new MessageWindow(null, "Preparacion para Envio", "No estan preparadas todas las transacciones en este rango de fechas");
						wm.setVisible(true);			    	        				    	    	
					}
					String msg = ctrl.checkTrxData(0);
					if (msg != null) {
						sendData = false;
						ErrorMessageWindow x = new ErrorMessageWindow(null, msg);
						x.setVisible(true);		
					}
					*/
					if (sendData == true) {
						...
						if (m.getLevel().equalsIgnoreCase(ApplicationMessage.MESSAGE)) {
							MessageWindow wm = new MessageWindow(null, "Envio de Lote de Facturas", m.getText());
							wm.setVisible(true);	
							// ejecutar inmediatamente la consulta de los lotes enviados
							//
							m = ctrl.queryBatches();
							if (m.getLevel().equalsIgnoreCase(ApplicationMessage.MESSAGE)) {
								wm = new MessageWindow(null, "Consulta de Lote de Facturas", m.getText());
								wm.setVisible(true);	
							} else {
			    ...
			    public void mouseClicked(MouseEvent e) {  
		            ...
		            if (rowIndex > -1) {
			            // el CDC de la transaccion se encuentra en la columna 9
	            	        txNumber = (String) jaTrxGroups.getValueAt(rowIndex, 0);
		            	    controlCode = (String) jaTrxGroups.getValueAt(rowIndex, 9);
		            	    transactionId = (long) jaTrxGroups.getValueAt(rowIndex, 10);
		            	    System.out.println("CDC: " + controlCode + " No.: " + txNumber + " Id.: " + transactionId);
		            	    if (controlCode != null) {
		                    ctrl.setSelectedCtrlCode(controlCode);
		    	                DocumentoElectronico d = ctrl.queryCDC ( controlCode, transactionId, txNumber );
		                    //
		    	                if (d != null) {
						        openDataWindow(d);
		    	                }
	...
	private void getjaTrxGroups ( String viewType ) {
		if (viewType.equalsIgnoreCase("INVOICES")) {
			jaTrxGroups = new JTable(ctrl.getInvoicesTableModel());
	...
	private JTable getEmptyTable ( ) {
		...
		//
		tm = ctrl.createInvoicesEmptyTable();
    ...
    private void queryInvoices () {
	    ...
	    //x.setVisible(true);  
    	    ctrl.loadInvoicesDataModel();
    	    ctrl.createInvoicesItemsTable();
		if (jsTrxGroups == null) {
			System.out.println("jsTransactions es nulo...");
		    jpCenter.add(getjsTrxGroups("INVOICES"), null);
		} else {
			System.out.println("jsTransactions no es nulo...");
		    getjaTrxGroups ("INVOICES");
		    jsTrxGroups.setViewportView(jaTrxGroups);
    ...
    private void querySendDocuments () {
	    //ErrorMessageWindow x = new ErrorMessageWindow(null, "Ejecutando consulta");
	    //x.setVisible(true);  
    	    ctrl.loadSentDataModel();
    	    ctrl.createInvoicesItemsTable();
		if (jsTrxGroups == null) {
			System.out.println("jsTransactions es nulo...");
		    jpCenter.add(getjsTrxGroups("INVOICES"), null);
	...
	public static void main(String[] args) {
		...
		//
		SendRcvInvoicesBeta w = new SendRcvInvoicesBeta();
		w.setVisible(true);
		w.repaint();
	}
