...
import business.ResendPosInvoicesControllerBeta;
...
public class ResendPosInvoicesBeta extends JFrame {
	...
	private ResendPosInvoicesControllerBeta ctrl;
	...
	public ResendPosInvoicesBeta () {
	    ...
	    this.ctrl = new ResendPosInvoicesControllerBeta();
	...
	private JPanel getJpToolbar() {
		if (jpToolbar == null) {
			...
			jlAction1.setText(" Ver Facturas");
			...
			jlAction1.addMouseListener(new MouseAdapter() {  
				public void mouseClicked(MouseEvent e) {  
					if (ctrl.getTrxDate() == null) {
						ErrorMessageWindow x = new ErrorMessageWindow(null, "Debe indicar la fecha para listar las facturas");
						x.setVisible(true);			    	        				    	    				    	    	
					} else {
			...
			jlAction2.setText(" Preparar Datos");
			...
			jlAction2.addMouseListener(new MouseAdapter() {  
				public void mouseClicked(MouseEvent e) {  
					if (ctrl.getTrxDate() == null) {
						...
						//
						if (prepareTrx == true) {
							int qty = ctrl.deleteTrx();
						    MessageWindow x = new MessageWindow(null, "Eliminacion de Transacciones", qty + " transaciones eliminadas");
						    x.setVisible(true);	
						    //
							ApplicationMessage m2 = ctrl.prepareTrx();
							if (m2.getLevel().equalsIgnoreCase(ApplicationMessage.ERROR) == true) {
								ErrorMessageWindow em = new ErrorMessageWindow(null, m2.getText());
								em.setVisible(true);		
							} else {
								...
								jtFromGroup.repaint();
								jtToGroup.repaint();
								MessageWindow wm = new MessageWindow(null, "Preparacion de Transacciones", m2.getText());
								wm.setVisible(true);	
							}
						}
					}  
			...
			jlAction3.addMouseListener(new MouseAdapter() {  
			    public void mouseClicked(MouseEvent e) {  
	    	            ApplicationMessage m = ctrl.sendTxBatch ( ctrl.getFromGroup(), ctrl.getToGroup() );
			...
			jlAction4.setText(" Consultar Documento");
			...
			jlAction4.addMouseListener(new MouseAdapter() {  
			    public void mouseClicked(MouseEvent e) {  
		            ...
		            if (rowIndex > -1) { 
			            // el CDC de la transaccion se encuentra en la columna 7
	            	        txNumber = (String) jaTrxGroups.getValueAt(rowIndex, 1);
		            	    controlCode = (String) jaTrxGroups.getValueAt(rowIndex, 7);
		            	    transactionId = (long) jaTrxGroups.getValueAt(rowIndex, 8);
		            	    cashId = (long) jaTrxGroups.getValueAt(rowIndex, 9);
		            	    controlId = (long) jaTrxGroups.getValueAt(rowIndex, 10);
		            	    System.out.println("CDC: " + controlCode + " No.: " + txNumber + " Id.: " + transactionId);
		            	    if (controlCode != null) {
		                    ctrl.setSelectedCtrlCode(controlCode);
		    	                DocumentoElectronico d = ctrl.queryCDC ( controlCode, transactionId, controlId, cashId, txNumber );
		                    //
		    	                if (d != null) {
						        openDataWindow(d);
		    	                }
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					if (aMsg == null) {
						if (ctrl.getTrxDate() != null) {
							...
							jtDate.setText(d);
							// mostrar la tabla de transacciones no enviadas
							queryInvoices();
						} else {
							jtDate.setText("");							
						}
	...
	private void openDataWindow ( DocumentoElectronico d ) {
		ViewElectronicDocumentCtrl ctrl = new ViewElectronicDocumentCtrl();
		ctrl.setDE(d);
	    ViewElectronicDocument w = new ViewElectronicDocument( this, ctrl );
	    w.setVisible(true);			
	}
	...
	public static void main(String[] args) {
		...
		//
		ResendPosInvoicesBeta w = new ResendPosInvoicesBeta();
		w.setVisible(true);
		w.repaint();
	}
