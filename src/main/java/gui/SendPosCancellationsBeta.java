...
import business.SendPosCancellationsController;
import business.SessionContext;
import business.UserAttributes;
import pojo.ListOfValuesSelection;

public class SendPosCancellationsBeta extends JFrame {
	...
	private SendPosCancellationsController ctrl;
	 ...
	 */
	public SendPosCancellationsBeta () {
		super();
	    ...
	    this.ctrl = new SendPosCancellationsController();
		// build GUI
		initialize();
		try {		
			...
			jlAction1.addMouseListener(new MouseAdapter() {  
			    public void mouseClicked(MouseEvent e) {  
			    	    if (ctrl.getCanceledTxList().size() > 0) {
							...
							MessageWindow x = new MessageWindow(null, "Evento Cancelacion", m.getText());
							x.setVisible(true);	
			    	        }
			    	    } else {
	...
	private void getjaTransactions ( ) {
		...
		for ( int i=0; i<jaTransactions.getColumnCount(); i++) {
			...
			if ( i == 4 ) {
				// numero de documento de cliente
				tc.setPreferredWidth(50);
			}
			if ( i == 5 ) {
				// denominacion de cliente
				tc.setPreferredWidth(80);
			}
			if ( i == 6 ) {
				// CDC
	...
	public static void main(String[] args) {
		...
		//
		SendPosCancellationsBeta w = new SendPosCancellationsBeta();
		w.setVisible(true);
		w.repaint();
	}
