...
import business.PosTrxEbBatchItemsTM;
import business.PosTrxEbBatchesTM;
import business.SessionContext;
import business.UserAttributes;
import business.ViewPosTrxBatchItemsCtrl;
import business.ViewPosTrxBatchesCtrl;
...
public class ViewPosTrxBatches extends JFrame {
	...
	private JScrollPane jsBatchItems = null;
	...
	private ViewPosTrxBatchesCtrl ctrl;
	
	// Application specific data
	private final String appModule = "Avanza Electronic Billing";
	
	private int itemsHits;
		
	/**
	 * This is the default constructor
	 */
	public ViewPosTrxBatches () {
		super();
	    ...
	    this.ctrl = new ViewPosTrxBatchesCtrl();
	    itemsHits = 0;
		// build GUI
		initialize();
		try {		
	...
	private JPanel getJpToolbar() {
		if (jpToolbar == null) {
			    ...
			    public void mouseClicked(MouseEvent e) {  
		            ...
		            if (rowIndex > -1) {
			            // el identificador del objeto a editar se encuentra en la columna 6
		            	    batchNumber = (String) jaDocBatches.getValueAt(rowIndex, 0);
		            	    batchId = (long) jaDocBatches.getValueAt(rowIndex, 8);
		            	    itemsQty = (int) jaDocBatches.getValueAt(rowIndex, 6);
		            	    System.out.println("batchNumber: " + batchNumber);
		                ctrl.setSelectedBatchNo(batchNumber);
		    	            ApplicationMessage m = ctrl.querySingleBatch ( batchNumber, batchId, itemsQty );
		    	            // consultar y mostrar la tabla de elementos del lote
			            ...
			            ErrorMessageWindow x = new ErrorMessageWindow(null,"Debe elegir un lote para importar");
			            x.setVisible(true);			    	        	
		            }
		        }  
				public void mouseEntered (MouseEvent evt) {
			...
			jlAction3.setText(" Importar Todo");
			...
			jlAction3.addMouseListener(new MouseAdapter() {  
			    public void mouseClicked(MouseEvent e) {  
            	        // consultar todos los lotes cuyos valores seleccionados coincidan con los valores
            	        // de los filtros de la consulta
	                ApplicationMessage m = ctrl.queryBatchesGroup ( );
	                if (m.getLevel().equalsIgnoreCase(ApplicationMessage.MESSAGE)) {
	        	            MessageWindow x = new MessageWindow(null, "Consulta de Lote de Documentos", m.getText());
			            x.setVisible(true);			    	        	
	                } else {
	...
	private void openItemsWindow ( long batchId, String batchNumber ) {
		itemsHits++;
		System.out.println("Numero de consulta: " + itemsHits);
		ViewPosTrxBatchItemsCtrl ctrl = new ViewPosTrxBatchItemsCtrl();
		ctrl.setBatchId(batchId);
		ctrl.setBatchNumber(batchNumber);
	    ViewPosTrxBatchItems w = new ViewPosTrxBatchItems( this, ctrl );
	    w.setVisible(true);
	    System.out.println("reclamando foco...");
	    jtBatchNumber.requestFocusInWindow();
	}
	...
	public static void main(String[] args) {
		...
		//
		ViewPosTrxBatches w = new ViewPosTrxBatches();
		w.setVisible(true);
		w.repaint();
	}
