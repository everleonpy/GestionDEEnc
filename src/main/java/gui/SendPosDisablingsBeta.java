...
import business.PosDisabledInvoicesTM;
import business.SendPosDisablingsController;
import business.SessionContext;
import business.UserAttributes;
import pojo.ListOfValuesSelection;

public class SendPosDisablingsBeta extends JFrame {
	...
	private SendPosDisablingsController ctrl;
	...
	public SendPosDisablingsBeta () {
	    ...
	    this.ctrl = new SendPosDisablingsController();
		// build GUI
		initialize();
		try {		
	...
	private void initialize() {
		...
		this.setName("SendPosDisablingsBeta");
		this.setResizable(false);
		this.setLayout(new BorderLayout());
		//this.setContentPane(new JLabel(new ImageIcon(AppConfig.iconsFolder + "main-bg.jpg")));
		this.add(getJContentPane());
		//
		this.setTitle("Inutilizacion de Numeros de Comprobantes POS");
	...
	private JPanel getJpToolbar() {
		if (jpToolbar == null) {
			...
			jlAction1.setText(" Ver Transacciones");
			...
			jlAction1.addMouseListener(new MouseAdapter() {  
			    public void mouseClicked(MouseEvent e) {  
					    ...
					    ErrorMessageWindow x = new ErrorMessageWindow(null, "Debe indicar la fecha para la consulta");
					    x.setVisible(true);			    	    	
			    	    }
			    }  
			...
			jlAction2.setText(" Enviar Evento");
			...
			jlAction2.addMouseListener(new MouseAdapter() {  
			    public void mouseClicked(MouseEvent e) {  
		    	        ...
		    	        if (ctrl.getDisabledTxList() != null) {
		    	        	System.out.println("2");
			    	        if (ctrl.getDisabledTxList().size() > 0) {
			    	        	System.out.println("3");
			    	    	        ApplicationMessage m = ctrl.sendEvents();
			    	    	        if (m != null) {
			    	    	    	        if (m.getLevel().equalsIgnoreCase(ApplicationMessage.ERROR)) {
							        ErrorMessageWindow x = new ErrorMessageWindow(null, m.getText());
							        x.setVisible(true);			    	    	
			    	    	    	        } else {
							        MessageWindow x = new MessageWindow(null, "Evento inutilizacion", m.getText());
								    x.setVisible(true);			    	    				    	    	    	    	
			    	    	    	        }
			    	    	        } else {
    ...
    private void executeQuery () {
		...
		jpCenter.repaint();
        MessageWindow x = new MessageWindow(null, "Inutilizacion de Numeros", "Cantidad de transacciones: " + ctrl.getDisabledTxList().size());
	    x.setVisible(true);			    	    				    	    	    	    	
	...
	public static void main(String[] args) {
		...
		//
		SendPosDisablingsBeta w = new SendPosDisablingsBeta();
		w.setVisible(true);
		w.repaint();
	}
