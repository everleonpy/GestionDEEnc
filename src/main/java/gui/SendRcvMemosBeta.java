...
import business.RcvMemosTM;
import business.SendRcvMemosControllerBeta;
...
public class SendRcvMemosBeta extends JFrame {
	...
	private SendRcvMemosControllerBeta ctrl;
		...
		if (jpToolbar == null) {
			...
			jlAction1.addMouseListener(new MouseAdapter() {  
				public void mouseClicked(MouseEvent e) {  
					if (ctrl.getTrxDate() == null) {
						ErrorMessageWindow x = new ErrorMessageWindow(null, "Debe indicar la fecha para listar las notas");
						x.setVisible(true);			    	        				    	    				    	    	
					} else {
						boolean viewTrx = true;
						// determinar si existen transacciones de tipo nota en el rango de fechas
						ctrl.findMemosCount();
						if (ctrl.getMemosQty() == 0) {
							viewTrx = false;
				...
				public void mouseClicked(MouseEvent e) {  
					if (ctrl.getTrxDate() == null) {
						...
						if (ctrl.getMemosQty() == 0) {
							...
							if (ctrl.getPreparedQty() == ctrl.getMemosQty()) {
								prepareTrx = false;
			...
			jlAction3.addMouseListener(new MouseAdapter() {  
				public void mouseClicked(MouseEvent e) {  
					if (ctrl.getTrxDate() == null) {
						...
						ctrl.findPreparedCount();
						if (ctrl.getPreparedQty() < ctrl.getMemosQty()) {
							ErrorMessageWindow x = new ErrorMessageWindow(null, "No estan preparadas todas las transacciones en este rango de fechas");
							x.setVisible(true);			    	        				    	    	
						}
						queryPrepared();			    	        
					}  
			...
			jlAction4.addMouseListener(new MouseAdapter() {  
				public void mouseClicked(MouseEvent e) {  
					...
					// verificar si existen datos preparados para el envio
					if (ctrl.getMemosQty() == 0) {
						ctrl.findMemosCount();
					}
					if (ctrl.getPreparedQty() < ctrl.getMemosQty()) {
						//sendData = false;
						ErrorMessageWindow em = new ErrorMessageWindow(null, "No estan preparadas todas las transacciones en este rango de fechas");
						em.setVisible(true);			    	        				    	    	
					}
					//
					if (sendData == true) {
						...
						if (m.getLevel().equalsIgnoreCase(ApplicationMessage.MESSAGE)) {
							MessageWindow wm = new MessageWindow(null, "Envio de Lote de Notas", m.getText());
							wm.setVisible(true);	
							// ejecutar inmediatamente la consulta de los lotes enviados
							m = ctrl.queryBatches();
							if (m.getLevel().equalsIgnoreCase(ApplicationMessage.MESSAGE)) {
								wm = new MessageWindow(null, "Consulta de Lote de Notas", m.getText());
								wm.setVisible(true);	
							} else {
			...
			jlAction5.addMouseListener(new MouseAdapter() {  
			    public void mouseClicked(MouseEvent e) {  
		            ...
		            if (rowIndex > -1) {
			            // el CDC de la transaccion se encuentra en la columna 8
		            	    controlCode = (String) jaTrxGroups.getValueAt(rowIndex, 8);
	...
	private JPanel getJpFilters() {
		if (jpFilters == null) {
			...
			lineNum++;
			lbY = lbY + lbH;
			jlTxNumber = getFiltersLabel("No. Nota");
	...
	private void getjaTrxGroups ( String viewType ) {
		if (viewType.equalsIgnoreCase("MEMOS")) {
			jaTrxGroups = new JTable(ctrl.getMemosTableModel());
		...
		if (viewType.equalsIgnoreCase("PREPARED")) {
			...
			jaTrxGroups.setRowHeight(30);
			jaTrxGroups.setFont(appLook.getSmallFont());
			//{"No. Item", "No. Nota", "Fecha", "Importe", "RUC/No. Identidad", 
    ...
    private void queryMemos () {
	    ...
	    //x.setVisible(true);  
    	    ctrl.loadMemosDataModel();
    	    ctrl.createMemosItemsTable();
		if (jsTrxGroups == null) {
			System.out.println("jsTransactions es nulo...");
		    jpCenter.add(getjsTrxGroups("MEMOS"), null);
		} else {
			System.out.println("jsTransactions no es nulo...");
		    getjaTrxGroups ("MEMOS");
		    jsTrxGroups.setViewportView(jaTrxGroups);
	...
	public static void main(String[] args) {
		...
		//
		SendRcvMemosBeta w = new SendRcvMemosBeta();
		w.setVisible(true);
		w.repaint();
	}
