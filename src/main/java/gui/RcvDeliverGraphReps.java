...
import business.RcvDeliverDocumsTM;
...
public class RcvDeliverGraphReps extends JFrame {
	...
	private void initialize() {
		...
		this.setName("RcvDeliverGraphReps");
		this.setResizable(false);
		this.setLayout(new BorderLayout());
		//this.setContentPane(new JLabel(new ImageIcon(AppConfig.iconsFolder + "main-bg.jpg")));
		this.add(getJContentPane());
		//
		this.setTitle("Envio de Representaciones Graficas de Facturas Mayoristas");
		...
		if (jpToolbar == null) {
			...
			jlAction1.addMouseListener(new MouseAdapter() {  
			    public void mouseClicked(MouseEvent e) {  
			        ...
			        if (rowIndex >= 0) {
			            ...
			            eMail = (String) jaTransactions.getValueAt(rowIndex, 1);
			            fileName = (String) jaTransactions.getValueAt(rowIndex, 1);
			            ctrl.setSelectedTransactionId(invoiceId);
			    	        ApplicationMessage m = ctrl.deliverGraphRepresent(invoiceId, controlId, cashId, txNumber, eMail, fileName);
			...
			jlAction2.addMouseListener(new MouseAdapter() {  
			    public void mouseClicked(MouseEvent e) {  
		            ...
		            if (errCounter == 0) {
	        	            MessageWindow x = new MessageWindow(null, "Envio de KuDE", "Todos los KuDE han sido enviados");
			            x.setVisible(true);
		            } else {
			            ErrorMessageWindow x = new ErrorMessageWindow(null, errCounter + " KuDE no han podido ser enviados");
			            x.setVisible(true);		            	
		            }
			    }  
			    public void mouseEntered (MouseEvent evt) {
	...
	private void getjaTransactions ( ) {
		...
		for ( int i=0; i<jaTransactions.getColumnCount(); i++) {
			...
			if ( i == 5 ) {
				// archivo que contiene la representacion grafica
				tc.setPreferredWidth(150);
			}
			if ( i == 6 ) {
			...
			if ( i == 8 ) {
				// identificador de control de caja
				tc.setPreferredWidth(30);
			}
		}
	}	
	...
	private JTable getEmptyTable ( ) {
		...
		//
		tm = ctrl.createDeliverEmptyTable();
	...
	public static void main(String[] args) {
		...
		//
		RcvDeliverGraphReps w = new RcvDeliverGraphReps();
		w.setVisible(true);
		w.repaint();
	}
