...
public class SendPosInvoicesBeta extends JFrame {
	...
	SendPosInvoicesAsync task;
	private Timer processMonitor;
	...
	private JPanel getJpToolbar() {
		if (jpToolbar == null) {
			...
			jlAction1.setText(" Preparar RUCs");
			...
			jlAction1.addMouseListener(new MouseAdapter() {  
				public void mouseClicked(MouseEvent e) {  
					if (ctrl.getTrxDate() == null) {
						ErrorMessageWindow x = new ErrorMessageWindow(null, "Debe indicar la fecha para preparar los RUCs");
						x.setVisible(true);			    	        				    	    				    	    	
					} else {
						ApplicationMessage m = ctrl.prepareTaxNumbers();
						if (m.getLevel().equalsIgnoreCase(ApplicationMessage.ERROR)) {
							ErrorMessageWindow x = new ErrorMessageWindow(null, m.getText());
							x.setVisible(true);		
						} else {
							MessageWindow x = new MessageWindow(null, "Preparacion de RUCs", m.getText());
							x.setVisible(true);	
						}
					}  
				}
			...
			jlAction3.addMouseListener(new MouseAdapter() {  
			    public void mouseClicked(MouseEvent e) {  
				    ...
				    task = new SendPosInvoicesAsync ( ctrl.getTRX_TYPE(), 
 		                                              ctrl.getTrxDate(), 
 		                                              ctrl.getFromGroup(), 
 		                                              ctrl.getToGroup(),
 		                                              ctrl.getPosOpts().getEbBatchTxQty(),
 		                                              UserAttributes.userOrg.getIDENTIFIER(), 
 		                                              UserAttributes.userUnit.getIDENTIFIER(), 
 		                                              UserAttributes.userName);
				    task.start();

				    processMonitor = new Timer (2000, new ActionListener () {
	...
	private JPanel getJpFilters() {
		if (jpFilters == null) {
			...
			lbX = startX;
			lbY = lbY + lbH;
			jlCurrentTask = getLongLabel("Ninguna tarea en ejecucion");
			jpFilters.add(jlCurrentTask, null);
			/**
			 +----------------------------------------------------+
	...
	private JLabel getLongLabel ( String text ) {
		JLabel l = new JLabel();
		l.setHorizontalTextPosition(SwingConstants.LEFT);
		l.setHorizontalAlignment(SwingConstants.LEFT);
		l.setSize(new Dimension((lbW * 6), lbH));
		l.setLocation(new Point(lbX, lbY));
		l.setForeground(appLook.getBodyFg());
		l.setFont(appLook.getSmallFont());
		l.setText(text);
	...
	private void showTaskProgress () {
		...
		if (task.isAlive()) {
		    taskName = task.getCurrentTask();
			jlCurrentTask.setText(taskName);
			jlCurrentTask.repaint();	
		} else {
			if (processMonitor.isRunning()) {
			    processMonitor.stop();
			    processMonitor = null;
			}		
			jlCurrentTask.setText("Ninguna tarea en ejecucion.");
			jlCurrentTask.repaint();	
			//JOptionPane.showMessageDialog(null, "Tarea finalizada", "Informacion - Punto de Ventas", JOptionPane.INFORMATION_MESSAGE);
	        MessageWindow x = new MessageWindow(null, "Envio de Lote de Facturas", "Actividad realizada con exito");
            x.setVisible(true);			    	        	
		}
	}
	...
	public static void main(String[] args) {
		...
		//
		SendPosInvoicesBeta w = new SendPosInvoicesBeta();
		w.setVisible(true);
		w.repaint();
	}
